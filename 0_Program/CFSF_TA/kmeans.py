...
class kClusterer:
    ...
    def __init__(self, data_input, k):
        ...
        for line in lines:
            ...
            for cell in range(self.cols):
                if toggle == 0:
                   self.data[cell].append(int(cells[cell]))
                   toggle = 1
                else:
                    self.data[cell].append(float(cells[cell]))

        # mylib.print_matrix("USER-CLUSTERING", self.data)
    ...
    def updateCentroids(self):
        ...
        self.centroids = [
        [0 if members[centroid] == 0 else sum([self.data[k][i] for i in range(len(self.data[0])) if self.memberOf[i] == centroid])/members[centroid] for k in range(1, len(self.data))] for centroid in range(len(self.centroids))] 
    ...
    def euclideanDistance(self, i, j):
        ...
        dist = mylib.pearsonr(self.data[i], self.data[j])
        print "dist {0} - {1} : {2}".format(i,j,dist)
        return dist
    ...
    def run(self):
        ...
        while done:
            ...
            self.assignPointsToCluster()
            print "iteration", self.iterationNumber
            #
            # we are done if fewer than 1% of the points change clusters
            #
            #
            if float(self.pointsChanged) / len(self.memberOf) <  0.001:
                done = True
    ...
    def listCluster(self):
        return [[name for name in [self.data[0][i] for i in range(len(self.data[0])) if self.memberOf[i] == centroid]] for centroid in range(len(self.centroids))]
