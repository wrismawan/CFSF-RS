...
class kClusterer:
    ...
    def updateCentroids(self):
        ...
        self.centroids = [
            [0 if members[centroid] == 0 else sum(
                [self.data[k][i] for i in range(len(self.data[0])) if self.memberOf[i] == centroid]) / members[centroid]
             for k in range(1, len(self.data))] for centroid in range(len(self.centroids))]
    ...
    def listCluster(self):
        return [[name for name in [self.data[0][i] for i in range(len(self.data[0])) if self.memberOf[i] == centroid]]
                for centroid in range(len(self.centroids))]
