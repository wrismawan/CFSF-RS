...
class CF(object):
    def __init__(self, max_user, max_movie, C, K, M):
        ...
        rating_query = Rating_base.select().where(Rating_base.user_id < self.max_user).where(
            Rating_base.movie_id < self.max_movie).order_by(Rating_base.movie_id).limit(self.max_movie)
    ...
    def calc_rcui(self, num_cluster, item):
        ...
        print '>>>',self.user_cluster[num_cluster]
    ...
    def clusterUser(self):
        ...
        writeToCSV(listData=self.user_cluster, fileName='output/user_cluster_ok.csv')
    ...
    def create_top_K(self, active_user):
        ...
        IClust = [i_cluster(sim=c.similarity_value, no=c.num_cluster) for c in
                  Icluster.select().where(Icluster.user_id == active_user).order_by(Icluster.similarity_value.desc())]
    ...
    def construct_local_matrix(self, active_user):
        self.user_cluster = readFromCSV('output/user_cluster_ok.csv')
...
if __name__ == "__main__":
    cf = CF(max_user=5, max_movie=5, C=3, K=5, M=8)

    cf.user_item = readFromCSV("output/user-item.csv")

    cf.clusterUser()
    cf.data_smoothing()

    print_matrix("USER-ITEM", cf.user_item)
    print_matrix("CLUSTER-USER", cf.user_cluster)
    print_matrix("RATING-SMOOTHING", cf.user_item)
